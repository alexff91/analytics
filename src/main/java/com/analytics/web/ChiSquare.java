  ...
  public void chiSquare() {
    try {
      ...
      exporterBean.setTableHeader(
          "Chi-square test of " + exporterBean.getSelectedXChi() + " and " + exporterBean
              .getSelectedYChi());
      int xIndexes = 0;
      int yIndexes = 0;
      ...
      for (int i = 0; i < exporterBean.getStatisticsColumnTemplate().size(); i++) {
        ...
        if (exporterBean.getStatisticsColumnTemplate().get(i)
            .equals(exporterBean.getSelectedXChi())) {
          xIndexes = i;
        }
        if (exporterBean.getStatisticsColumnTemplate().get(i)
            .equals(exporterBean.getSelectedYChi())) {
          yIndexes = i;
        }
      }
      ...
      properties.get(0)
          .add(exporterBean.getSelectedXChi() + " and " + exporterBean.getSelectedYChi());
      ...
      exporterBean.setContent(
          exporterBean.getContent() + "<tr><td style=\"border:1px solid black;\"></td>");
      ...
      exporterBean.setContent(exporterBean.getContent() + "<tr>");
      exporterBean
          .setContent(exporterBean.getContent() + "<th style=\"border:1px solid black;\"></th>");
      exporterBean
          .setContent(exporterBean.getContent() + "<th style=\"border:1px solid black;\"></th>");
      exporterBean
          .setContent(exporterBean.getContent() + "<td style=\"border:1px solid black;\" colspan = "
              + (xSet.size()) + "><b>" + exporterBean.getSelectedXChi() + "</b></td>");
      exporterBean
          .setContent(
              exporterBean.getContent() + "<th style=\"border:1px solid black;\"> Row Count </th>");
      exporterBean.setContent(exporterBean.getContent() + "</tr>");
      exporterBean.setContent(exporterBean.getContent() + "<tr>");
      exporterBean
          .setContent(exporterBean.getContent() + "<th style=\"border:1px solid black;\"></th>");
      exporterBean
          .setContent(
              exporterBean.getContent() + "<th style=\"border:1px solid black;\">Row Values</th>");
      ...
      for (int i = 0; i < crosstab.table.length; i++) {
        ...
        if (i == 0) {
          exporterBean.setContent(
              exporterBean.getContent() + "<td style=\"border:1px solid black;\" rowspan="
        ...
        exporterBean.setContent(
            exporterBean.getContent() + "<td style=\"border:1px solid black;\"rowspan=" + 4
        ...
        for (int j = 0; j < crosstab.table[i].length; j++) {
          exporterBean.setContent(
              exporterBean.getContent() + "<td style=\"border:1px solid black;\"> Count "
                  + crosstab.table[i][j] + "</td>");
          rowcount += crosstab.table[i][j];
          if (i == 0) {
            chartYSeries
                .set("ColumnCount " + exporterBean.getSelectedXChi() + " " + xSet.toArray()[j],
                    columnCount.get(j));
          }
        }
        chartXSeries
            .set("Rowcount " + exporterBean.getSelectedYChi() + " " + ySet.toArray()[i], rowcount);
        exporterBean
            .setContent(
                exporterBean.getContent() + "<td style=\"border:1px solid black;\" rowspan = 4 >"
        ...
        for (int j = 0; j < crosstab.table[i].length; j++) {
          exporterBean
              .setContent(exporterBean.getContent() + "<td style=\"border:1px solid black;\" >"
        ...
        for (int j = 0; j < crosstab.table[i].length; j++) {
          exporterBean
              .setContent(exporterBean.getContent() + "<td style=\"border:1px solid black;\">"
                  ...
                  .roundTo2Decimals(
                      ((crosstab.table[i][j]) / new Double(columnCount.get(j))) * 100d)
                  + "</td>");
        }
        exporterBean.setContent(exporterBean.getContent() + "</tr>");
        exporterBean.setContent(exporterBean.getContent() + "<tr>");
        for (int j = 0; j < crosstab.table[i].length; j++) {
          ...
          if (stdResidual <= -1) {
            residualDifferenceColorStyle = "background: rgba(205, 87, 71, 0.79);";
          }
          if (stdResidual <= -2) {
            residualDifferenceColorStyle = "background: rgba(232, 36, 11, 0.81);";
          }

          if (stdResidual >= 1) {
            residualDifferenceColorStyle = "background: rgba(65, 202, 146, 0.56);";
          }
          if (stdResidual >= 2) {
            residualDifferenceColorStyle = "background: rgba(33, 202, 31, 0.85);";
          }

          exporterBean.setContent(
              exporterBean.getContent() + "<td style=\"border:2px solid black;"
      ...
      if (x.length == y.length) {
        properties.get(0)
            .add(String.valueOf(ExporterBean.roundTo2Decimals(TestUtils.chiSquare(x, y))));
        properties.get(0)
            .add(String.valueOf(ExporterBean.roundTo2Decimals(TestUtils.chiSquareTest(x, y))));
        properties.get(0)
            .add(String
                .valueOf(TestUtils.chiSquareTest(x, y, Double.valueOf(exporterBean.getAlpha()))));
      }
      ...
      exporterBean.setContent(exporterBean.getContent() + "<tr>");
      exporterBean.setContent(
          exporterBean.getContent() + "<td style=\"border:1px solid black;\" colspan = 5>"
              + "Total column count <b>" + columnCount + "<b/></td>");
      exporterBean.setContent(exporterBean.getContent() + "</tr>");
      exporterBean.setContent(
          exporterBean.getContent() + "<tr><td style=\"border:1px solid black;\"></td>");
      exporterBean.setContent(
          exporterBean.getContent() + "<th style=\"border:1px solid black;\">" + "</th>");
      exporterBean.setContent(exporterBean.getContent() + "<th style=\"border:2px solid black;\" " +
          "colspan = 3>"
          + crosstab.output + "</th>");
      exporterBean.setContent(exporterBean.getContent() + "</tr>");
      ...
      exporterBean.setContent(exporterBean.getContent()
          ...
          + "    <th style=\"border:1px solid black;background: rgba(232, 36, 11, 0.81);\"><-2.0</th> \n"
          + "    <th style=\"border:1px solid black;background: rgba(205, 87, 71, 0.79);\"><-1.0</th> \n"
          ...
          + "    <th style=\"border:1px solid black;background: rgba(65, 202, 146, 0.56);\">>1.0</th> \n"
          + "    <th style=\"border:1px solid black;background: rgba(33, 202, 31, 0.85);\">>2.0</th> \n"
          + "  </tr>\n"
          + "  <tr>\n"
      ...
      FacesContext.getCurrentInstance()
          .addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "Error!", e
