...
public class ChiSquare implements Serializable {
  ...
  public void chiSquare() {
    try {
      ...
      exporterBean.setContent("<div id=\"correlations\">"
          + "<h1>Crosstable</h1>");
      ...
      exporterBean.setContent(
          exporterBean.getContent() + "<th style=\"border:1px solid black;\" colspan = 4 >"
      ...
      exporterBean
          .setContent(exporterBean.getContent() + "<th style=\"border:1px solid black;\"></th>");
      exporterBean.setContent(
          exporterBean.getContent() + "<td style=\"border:1px solid black;\" colspan = " + (xSet
              .size()) +
              ">" + exporterBean.getSelectedXChi() + "</td>");
      exporterBean.setContent(
      ...
      while (iteratorX.hasNext()) {
        ...
        exporterBean
            .setContent(exporterBean.getContent() + "<td style=\"border:1px solid black;\">" + (
                exporterBean.getMapOfColumns().get(xIndexes) == null ? next : exporterBean
                    .getMapOfColumns().get(xIndexes).get(next))
                + "</td>");
      }
      exporterBean
          .setContent(exporterBean.getContent() + "<th style=\"border:1px solid black;\"></th>");
      ...
      for (int i = 0; i < crosstab.table.length; i++) {
        ...
        if (i == 0) {
          exporterBean.setContent(
              exporterBean.getContent() + "<td style=\"border:1px solid black;\" rowspan=" + (
                  crosstab.table.length * 4)
                  + ">" + exporterBean.getSelectedYChi() + "</td>");
        }
        ...
        exporterBean.setContent(
            exporterBean.getContent() + "<td style=\"border:1px solid black;\"rowspan=" + 4 + ">"
                + (
                exporterBean.getMapOfColumns().get(yIndexes) == null ? next : exporterBean
                    .getMapOfColumns().get(yIndexes).get(next))
                + "</td>");
        int rowcount = 0;
        for (int j = 0; j < crosstab.table[i].length; j++) {
          exporterBean.setContent(
        ...
        for (int j = 0; j < crosstab.table[i].length; j++) {
          exporterBean.setContent(
              exporterBean.getContent() + "<td style=\"border:1px solid black;\">"
                  + "Percent % within " + exporterBean.getSelectedXChi()
      ...
      if (x.length == y.length) {
        ...
        properties.get(0).add(
            String.valueOf(TestUtils.chiSquareTest(x, y, Double.valueOf(exporterBean.getAlpha()))));
      }
      ...
      exporterBean.setContent(
          exporterBean.getContent() + "<th style=\"border:1px solid black;\" colspan = 3>"
              + crosstab.output + "</th>");
      exporterBean.setContent(exporterBean.getContent() + "</tr>");
