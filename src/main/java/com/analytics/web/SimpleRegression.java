...
public class SimpleRegression implements Serializable {
	...
	public void simpleRegression() {
		try {
			exporterBean.setTableHeader("Simple Regression of dependant variable " + exporterBean.getSelectedDep()
					+ " and independant variable " + exporterBean.getSelectedInDep());
			List<org.apache.commons.math3.stat.regression.SimpleRegression> decList = new LinkedList<org.apache.commons.math3.stat.regression.SimpleRegression>();

			int dependant = 0;
			int independant = 0;
			for (int i = 0; i < exporterBean.getStatisticsColumnTemplate().size(); i++) {
				if (exporterBean.getStatisticsColumnTemplate().get(i).equals(exporterBean.getSelectedInDep())) {
					independant = i;
				}
				if (exporterBean.getStatisticsColumnTemplate().get(i).equals(exporterBean.getSelectedDep())) {
					dependant = i;
				}
			}
			...
			String[] propertyNames = new String[] { "Intercept", "Slope", "Square Error", "N", "Intercept Std Err",
					"Sum Of Cross Products", "Sum Squared Errors", "Slope Std Err", "R-Square",
			...
			properties.get(0).add("dependant variable " + exporterBean.getSelectedDep() + " and independant variable "
			...
			for (int i = 0; i < decList.size(); i++) {
				...
				double interceptStdErr = ExporterBean.roundTo2Decimals(simpleRegression.getInterceptStdErr());
				properties.get(i).add(String.valueOf(interceptStdErr));
				double sumOfCrossProducts = ExporterBean.roundTo2Decimals(simpleRegression.getSumOfCrossProducts());
				properties.get(i).add(String.valueOf(sumOfCrossProducts));
				double sumSquaredErrors = ExporterBean.roundTo2Decimals(simpleRegression.getSumSquaredErrors());
				...
				double slopeConfidenceInterval = ExporterBean
						.roundTo2Decimals(simpleRegression.getSlopeConfidenceInterval());
				properties.get(i).add(String.valueOf(slopeConfidenceInterval));
				double totalSumSquares = ExporterBean.roundTo2Decimals(simpleRegression.getTotalSumSquares());
			...
			exporterBean.setDescription(""
					...
					+ "     intercept is true; otherwise 0." + "     <p>\n"
					...
					+ "     0 is returned.</p>\n" + "     <p>\n" + "     <strong>Preconditions</strong>: <ul>\n"
					+ "     <li>At least two observations (with at least two different x values)\n"
					+ "     must have been added before invoking this method." + "     returned.\n"
					+ "     </li></ul></p>\n" + "     "
					...
					+ "    <strong>Preconditions</strong>: <ul>\n"
					+ "    <li>At least two observations (with at least two different x values)\n"
					+ "    must have been added before invoking this method." + "    </li></ul></p>\n" + "    "
